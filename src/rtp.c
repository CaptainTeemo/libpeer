...
static int rtp_encoder_encode_h264_fu_a(RtpEncoder *rtp_encoder, uint8_t *buf, size_t size) {
    ...
    while (size > 0) {
        ...
        if (size <= FU_PAYLOAD_SIZE) {
            ...
            rtp_encoder->on_packet(rtp_encoder->buf, size + sizeof(RtpHeader) + sizeof(NaluHeader) + sizeof(FuHeader),
...
void rtp_encoder_init(RtpEncoder *rtp_encoder, MediaCodec codec, RtpOnPacket on_packet, void *user_data) {
    ...
    switch (codec) {
        case CODEC_H264:
            ...
            rtp_encoder->timestamp_increment = 90000 / 25; // 25 FPS.
...
//    stream->on_packet((uint8_t *)packet, bytes, stream->user_data);
//    return 0;
//}

//void rtp_encoder_init(struct rtp_stream *stream, MediaCodec codec, RtpOnPacket on_packet, void *user_data) {
...
//    switch (codec) {
//        case CODEC_H264:
//            stream->encoder = rtp_payload_encode_create(112, "H264", 0, 1, &handler, stream);
//            stream->timestamp_increment = 90000 / 25; // 25 FPS.
//            break;
//        case CODEC_PCMA:
//            stream->encoder = rtp_payload_encode_create(RTP_PAYLOAD_PCMA, NULL, 0, 4, &handler, stream);
//            stream->timestamp_increment = AUDIO_LATENCY * 8000 / 1000;
//            break;
//        default:
//            break;
//    }
//}
//
//int rtp_encoder_encode(struct rtp_stream *stream, uint8_t *data, size_t size) {
//    stream->timestamp += stream->timestamp_increment;
//    return rtp_payload_encode_input(stream->encoder, (void *)data, size, stream->timestamp);
//}